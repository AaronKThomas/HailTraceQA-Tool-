  ...
  return (
    <>
      ...
      <div className="history-search-wrap stagger" style={{ animationDelay: "20ms" }}>
        ...
        <input placeholder="Search history…" value={search} onChange={(event) => setSearch(event.target.value)} />
      </div>
      <div className="history-filter-row stagger" style={{ animationDelay: "30ms" }}>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="tests-list">
          {filtered.map((entry, index) => {
            ...
            return (
              <div key={entry.id} className="history-item stagger" style={{ animationDelay: `${index * 10}ms`, background: config.bg, borderColor: config.border, borderLeftColor: config.dot }}>
                <span className="history-icon" style={{ color: config.dot }}>{icon}</span>
                <div className="history-body">
                  ...
                  <div className="history-footer">
                    ...
                    <button className="rerun-btn" style={{ fontSize: 11, padding: "2px 8px" }} onClick={() => onRerun(entry)}>↺ Re-run</button>
                  </div>
                </div>
              </div>
            );
